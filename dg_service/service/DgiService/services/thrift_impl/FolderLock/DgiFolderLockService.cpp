...
namespace service
{
	namespace thrift_impl
	{
		...
		void DgiFolderlockService::setState(::dgi::DgiResult& _return, const  ::dgi::DgiSid& _sid, const  ::dgi::utf8string& _flockPath, const  ::dgi::FLockState::type _newState)
		{
			...
			//
			::logic::folderlock::storage::FLockObject flockInfo;
			result = DgiEngine::getFlock().get(path, flockInfo);

			if (IntSuccess(result))
			{
				//if (_newState == flockInfo.state)
				//{
					// Ignore change of the state. I'll do that later.
				//	m_log.print(std::string(__FUNCTION__) + ": info - we change current state on the same state.");
			...
			{
				m_log.print(std::string(__FUNCTION__) + ": error - no information about the flock. This is an unknown error.");
