...
namespace logic
{
	namespace encryption
	{
		...
		bool FileEncoder::acquireFile(const std::wstring& _filepath, bool _readOnly, HANDLE& _hFile) const
		{
			...
			{
				DWORD dwLastErrorIs = GetLastError();
				m_log.print(fn + ": error - can't open the file, the last error is " + std::to_string(dwLastErrorIs));
				return false;
			}
		...
		logic::common::InternalStatus FileEncoder::isEncoded(const std::wstring& _filepath, bool& _result)
		{
			...
			{
				m_log.print(fn + ": error - can't open file " + strings::ws_s(_filepath));
			...
			EncodingMetaInfo & _information)
		{
			std::string fn = __FUNCTION__;
			HANDLE hFile = INVALID_HANDLE_VALUE;
			logic::common::InternalStatus result = logic::common::InternalStatus::Int_UnknownError;
