...
namespace logic
{
	namespace common
	{
			...
			m_hashStorage(_passinfofile)
		{
			...
			{
				// Zero memory in case we init MPR for first time.
				ZeroMemory((void*)m_hashStorage.getMappedData(), sizeof(MprInfo));
			}
		}
		...
		bool MPassKeeper::DidThePasswordAlreadyAlreadySet()
		{
			std::unique_lock<std::mutex> lock(m_lock);
