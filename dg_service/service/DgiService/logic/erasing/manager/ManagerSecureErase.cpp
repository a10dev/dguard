...
namespace logic
{
	namespace secure_erase
	{
		namespace manager
		{
			...
			{
				// Print any configuration errors in log only here.
				m_driverConfigured = configureDriver(true);
			}
			...
			logic::common::InternalStatus Shredder::secureEraseFile(std::wstring _filePath)
			{
				...
				//
				//	Just for test cases we can use DeleteFile instead of driver.
				//
#ifdef DONT_USE_DGI_FERASE_DRIVER
				...
				else
				{
					...
					m_log.print(std::string(__FUNCTION__) + ": success - " + strings::ws_s(_filePath) + " was deleted.");
				}
				...
				{
					// Try to remove file just with delete file. It's not right? What do you think, Elon?
					auto result = DeleteFileW(_filePath.c_str());
			...
			void Shredder::eraseDirHierarchy(const windir::DirEntry& _dir, std::vector<EraseObjectResult>& _outResultList)
			{
				...
				for (auto file : _dir.files)
				{
					...
					if (result != Int_Success)
					{
						...
						m_log.print(fnname + ": interrupt erasing process because of it.");
						return;
					}
				}
			...
			bool Shredder::configureDriver(bool _logAnyErrors)
			{
				...
				if (m_eraseDriver.Open())
				{
					...
					if (_logAnyErrors)
					{
						m_log.print(std::string(__FUNCTION__) + ": error - failed to configure shred driver with required options.");
					}
				}
				else
				{
