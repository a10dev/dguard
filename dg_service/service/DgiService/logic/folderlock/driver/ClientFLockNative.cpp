...
namespace driver
{
	...
	bool ClientFLock::setDbgPrintFlags(
		ULONG _flags,
		ULONG& _oldFlags
		)
	{
	...
	bool ClientFLock::readFileMeta(
		std::wstring _path,
		FLOCK_META& _metaInformation
		)
	{
		DWORD cdReturned = 0;
		HANDLE hDrv = getDriverHandle();
		...
		{
			m_log.printEx(
				"%s: error - could not reserve enough memory, required size is %d.",
				__FUNCTION__,
				inputSize);
	...
	bool ClientFLock::writeFileMeta(std::wstring _path, FLOCK_META& _meta)
	{
		...
		memcpy(&requestBody->info, &_meta, sizeof(FLOCK_META));
	...
	bool ClientFLock::makeMetaInvalid(std::wstring _path)
	{
		DWORD cdReturned = 0;
		HANDLE hDrv = getDriverHandle();
		FLOCK_REQUEST_HEADER request = buildNewRequest();
	...
	bool ClientFLock::ctxEnable(bool _toEnable)
	{
		HANDLE hDrv = getDriverHandle();
		FLOCK_REQUEST_HEADER request = buildNewRequest();
		FLOCK_RESPONSE_HEADER response = { 0 };
		...
		request.params.boolValue = _toEnable;

		BOOL res = DeviceIoControl(
			hDrv,
			IOCTL_FLOCK_CONTEXT_ENABLE,
			&request,
			sizeof(request),
			&response,
			sizeof(response),
	...
	bool ClientFLock::getDriverInfo(FLOCK_COMMON_INFO& _info)
	{
		HANDLE hDrv = getDriverHandle();
		FLOCK_REQUEST_HEADER request = buildNewRequest();
		RESPONSE_FLOCK_INTERNAL_INFO response = { 0 };
		DWORD cdReturned = 0;
	...
	bool ClientFLock::cacheReSize(ULONG _newTableSize)
	{
		return false;
	}

	bool ClientFLock::cacheEnable(bool _toEnable)
	{
		HANDLE hDrv = getDriverHandle();
		FLOCK_REQUEST_HEADER request = buildNewRequest();
		FLOCK_RESPONSE_HEADER response = { 0 };
		...
		request.params.boolValue = (_toEnable == true);
	...
	bool ClientFLock::cacheGetInfo(/* Return info here in arguments. */)
	{
		return false;
	}
